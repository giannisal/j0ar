...
public class Joar_DB {
	...
	public void open() throws SQLException {
		...
		try {
			con = DriverManager.getConnection("jdbc:mysql://195.251.249.131:3306/ismgroup77", "ismgroup77", "rx673g");
		} catch (Exception e2) {
	...
	/**
	 * Η μέθοδος getData λαμβάνει ως όρισμα το σύνολο των γραμμάτων που
	 * πληκτρολογεί ο χρήστης και επιστρέφει λίστα με τις πιθανές λέξεις που
	 * ξεκινάνε με αυτά τα γράμματα
	 *
	 * @param query
	...
	/**
	 * Η μέθοδος findWordForStem λαμβάνει ως όρισμα την ρίζα της λέξης που
	 * πληκτρολόγησε ο χρήστης και επιστρέφει
	 *
	 * @param word
	 *            η ρίζα της λέξης που πληκτρολόγησε ο χρήστης
	 *
	 * @return λίστα με το σύνολο των link που που περιέχουν τις παράγωγες της
	 *         ρίζας της λέξης που πληκτρολόγησε ο χρήστης
	 *
	 * @throws SQLException
	 *             (with the appropriate message) if any error occured.
	 */
	public List<Siteword> findWordForStem(String word) throws Exception {
		...
		try {
			...
			stmt = con.prepareStatement(selectSitewordQuery);
	...
	/**
	 * Η μέθοδος getRelativesForStem δέχεται ένα site από αυτά που επιστράφηκαν
	 * από την μέθοδο "findWordForStem" και επιστρέφει το σύνολο των παράγωγων
	 * λέξεων που βρέθηκαν στο συγκεκριμένο site
	 *
	 * @param word
	 *            η ρίζα της λέξης που πληκτρολόγησε ο χρήστης
	 * @param site
	 *            το site που στάλθηκε απο την stem.jsp το οποίο είναι ένα από
	 *            αυτα που επέστρεψε η "findWordForStem"
	 *
	 * @return λίστα με το σύνολο των link που που περιέχουν τις παράγωγες της
	 *         ρίζας της λέξης που πληκτρολόγησε ο χρήστης
	 *
	...
	/**
	 * Η μέθοδος getSites παίρνει ένα πίνακα που έχει τις λέξεις που εισήγαγε ο
	 * χρήστης και εκτελώντας το ανάλογο query που δέχεται σαν όρισμα επιστρέφει
	 * τα ανάλογα site που εμπεριέχουν τις λέξεις αυτές.
	 *
	 * @param query
	 *            το query που θα τρέξει η prepareStatement αποθηκευμένο σε
	 *            μεταβλητή τύπου String
	 * @param words
	 *            πίνακας που εμπεριέχει το σύνολο των λέξεων που πληκτρολόγησε
	 *            ο χρήστης αποθηκευμένες σε μεταβλητή τύπου String
	 * @param n
	 *            το σύνολο των λέξεων που περιέχει πίνακας με τις λέξεις
	 *
	 * @return λίστα με το σύνολο των link που περιέχουν τις λέξεις που
	...
	/**
	 * Η μέθοδος getRelatives δέχεται ένα site από αυτά που επιστράφηκαν από την
	 * μέθοδο "getSites" και το κατάλληλο query που λαμβάνει απο το results.jsp
	 * καθώς και τον πίνακα με τις λέξεις που πληκτρολόγησε ο χρήστης και το
	 * μέγεθος του πίνακα αυτού, και επιστρέφει το σύνολο των λέξεων που
	 * βρέθηκαν στο site αυτο από αυτές που πληκτρολόγησε ο χρήστης
	 *
	 * @param finalQuery
	 *            το query που θα τρέξει η prepareStatement αποθηκευμένο σε
	 *            μεταβλητή τύπου String
	 ...
	 *            το σύνολο των λέξεων που περιέχει πίνακας με τις λέξεις
	 * @param link
	 *            το site που στάλθηκε απο την results.jsp το οποίο είναι ένα
	 *            από αυτα που επέστρεψε η "getSites"
	 *
	 * @return λίστα με το σύνολο των λέξεων που περιέχει το site από αυτές που
	 *         πληκτρολόγησε ο χρήστης
	 * @throws Exception
	 *             (with the appropriate message) if any error occured.
	 *
	...
	/**
	 * Η μέθοδος findSite δέχεται ένα site και επιστρέφει τον τίτλο του και την
	 * περιγραφή του
	 *
	 * @param site
	...
	public Sites findSite(String site) throws Exception {
		...
		try {
			...
			stmt = con.prepareStatement(selectSite2Query);
			stmt.setString(1, site);
			// execute query
			rs = stmt.executeQuery();
			while (rs.next()) {
	...
	/**
	 * Η μέθοδος validTrending παίρνει ένα String με ένα σύνολο λέξεων που
	 * εισήγαγε ο χρήστης και εκτελώντας το ανάλογο query ελέγχει αν αυτό το
	 * σύνολο λέξεων έχει αναζητηθεί προηγουμένως και είναι trending.
	 *
	 * @param word
	 *            το σύνολο των λέξεων που πληκτρολόγησε ο χρήστης αποθηκευμένες
	 *            σε μεταβλητή τύπου String
	 *
	 * @return boolean για το αν τελικά υπήρχε η λέξη σε προηγούμενες
	 *         αναζητήσεις και είναι treinding
	 *
	 * @throws Exception
	 *             (with the appropriate message) if any error occured.
	 */
	...
	/**
	 * Η μέθοδος findTrending εμφανίζει από τη βάση δεδομένων τη λίστα με τα
	 * Trending διατεταγμένη με βάση την ημερομηνία αναζήτησης,συχνότητα
	 * αναζήτησης και τη σημερινή συχνότητα αναζήτησης .
	 *
	 * @param word
	 *            το σύνολο των λέξεων που πληκτρολόγησε ο χρήστης αποθηκευμένες
	 *            σε μεταβλητή τύπου String
	 *
	 * @return λίστα με τα Trending διατεταγμένη με βάση την ημερομηνία
	 *         αναζήτησης,συχνότητα αναζήτησης και τη σημερινή συχνότητα
	 *         αναζήτησης .
	 *
	 * @throws Exception
	 *             (with the appropriate message) if any error occured.
	...
	/**
	 * Η μέθοδος importTrending εισάγει το σύνολο των λέξεων στη βάση δεδομένων
	 * και στον πίνακα Trending.
	 *
	 * @param word
	 *            το σύνολο των λέξεων που πληκτρολόγησε ο χρήστης αποθηκευμένες
	 *            σε μεταβλητή τύπου String
	...
	public void importTrending(String word) throws Exception {
		...
		try {
			...
			stmt = con.prepareStatement(importTrendingQuery);
			stmt.setString(1, word);
			// execute query
			stmt.executeUpdate();
			stmt.close();
	...
	/**
	 * Η μέθοδος updateTrending ενημερώνει τον πίνακα Trending και συγκεκριμένα
	 * τη συχνότητα κάθε λέξης όταν αυτή αναζητήται και υπάρχει ήδη.
	 *
	 * @param word
	 *            το σύνολο των λέξεων που πληκτρολόγησε ο χρήστης αποθηκευμένες
	 *            σε μεταβλητή τύπου String
	...
	public void updateTrending(String word) throws SQLException {
		...
		try {
			...
			stmt = con.prepareStatement(updateTrendingQuery);
			...
			errorMessages = "Error while updating word frequency to the database: <br>" + e4.getMessage();
			throw new SQLException(errorMessages);
		}
	}
